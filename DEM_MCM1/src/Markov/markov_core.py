...
from .utils import load_parquet_as_timestep_dict, apply_species_mask
...
class Markov:
    ...
    def __init__(
        self,
        method: Union[str, PartitioningMethod] = "cartesian",
    ...
    def load_dem_data(
        ...
        logger.info(
            f"Chargement DEM depuis {prefix} "
    ...
    def get_coords(
        self,
        timestep_indices: Union[list[int], int] = [250],
    ) -> np.ndarray:
        """
    ...
    def fit_partitioner(
        ...
        """
        logger.info(
            f"Fit partitioner sur {coordinates.shape[0]} points..."
        )
        
        self.partitioner.fit(coordinates)
        
        logger.info(
            f"✅ Partitioner fitted: {self.partitioner.n_cells} états"
        )
    ...
    def build_initial_state_vector(
        self,
        timestep_index: int,
        species_mask: Optional[np.ndarray] = None,
        normalize: bool = True,
    ) -> StateVector:
        """
        ...
        if self.partitioner.n_cells == 0:
            raise RuntimeError(
                "Partitioner non entraîné. "
                "Appelez fit_partitioner() d'abord."
            )
        ...
        if normalize:
            ...
            if not is_valid:
                logger.warning(
                    f"⚠️  Normalisation échouée: "
                    f"∑φ={phi.sum()}, expected={total}"
                )
    ...
    def propagate_markov(
        ...
        if transition_matrix.shape[0] != transition_matrix.shape[1]:
            raise ValueError(
                "Matrice transition doit être carrée"
            )
        ...
        for t in range(1, n_steps + 1):
            ...
            if validate_normalization:
                ...
                if relative_error > 1e-3:
                    logger.warning(
                        f"⚠️  Pas {t}: écart normalisation = "
                        f"{relative_error*100:.4f}%"
                    )
        
        logger.info(f"✅ Propagation terminée")
