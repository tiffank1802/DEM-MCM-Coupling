...
\begin{table}[H]
  \centering
  \caption{Écart moyen $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ (adimensionnel) sur la fenêtre de prédiction, par méthode de découpage à nombre de cellules identique (10 cellules, chaîne homogène, $\tau = 157$).}
...
\begin{table}[H]
  \centering
  \caption{RSD DEM de la teneur en petites particules (adimensionnel) à quelques instants (centièmes de seconde), par méthode de découpage : le régime transitoire ($t < 157$) présente des valeurs hautes et erratiques ; la décroissance s'installe au-delà.}
...
L'étude de sensibilité menée sur les données DEM (figure~\ref{fig:etude_step_courbe}) balaie $step$ de 40 à 471 (découpage physique, 10 cellules, $NLT = 3$, $\tau = 157$) et mesure l'erreur moyenne $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$. On observe que les valeurs intermédiaires non commensurables avec la période du tambour ($step = 80$) dégradent l'erreur (0{,}086) : les blocs échantillonnent alors des phases de rotation différentes, et leur moyenne mélange des configurations angulaires hétérogènes. Les valeurs commensurables avec la période ($step = 157$, $314$) ramènent l'erreur autour de 0{,}06, chaque bloc couvrant un nombre entier de tours ; au-delà ($step = 471$), l'erreur remonte, la fenêtre totale d'apprentissage s'étirant sans gain statistique. La petite valeur $step = 40$ donne l'erreur la plus basse sur ce cas particulier, mais au prix d'un recouvrement partiel des fenêtres d'observation des blocs, qui corrèle les blocs entre eux et fragilise l'interprétation de la moyenne --- et, pour les chaînes inhomogènes, désynchronise les matrices de la période physique.
