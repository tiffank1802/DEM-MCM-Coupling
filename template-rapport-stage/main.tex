...
La figure~\ref{fig:comparaison_methodes_rsd} confronte, pour les quatre méthodes de découpage et à paramètres identiques ($\tau = 157$, deux blocs d'apprentissage), le RSD de la teneur en petites particules prédit par la chaîne homogène à la référence DEM ; l'écart moyen $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ sur toute la fenêtre de prédiction est reporté dans chaque cadran et synthétisé au tableau~\ref{tab:ecart_rsd_methodes}. La hiérarchie est nette : le découpage physique (6,4 points d'écart) et le découpage de Voronoï (10,2 points) suivent correctement le niveau d'homogénéisation atteint par la DEM, tandis que les maillages géométriques --- cartésien (20,7 points) et cylindrique (27,7 points) --- s'en écartent durablement, leurs cellules moyennant des zones de dynamiques hétérogènes.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/comparaison_methodes_rsd.png}
  \caption{Influence de la méthode de découpage sur la prédiction (données DEM réelles) : RSD DEM vs prédiction markovienne homogène pour les quatre méthodes, à paramètres identiques ($\tau = 157$, deux blocs d'apprentissage). L'écart moyen est indiqué dans le titre de chaque cadran.}
  \label{fig:comparaison_methodes_rsd}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Écart moyen $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ (en points de \%) sur la fenêtre de prédiction, par méthode de découpage (chaîne homogène, $\tau = 157$).}
  \label{tab:ecart_rsd_methodes}
  \begin{tabular}{lcccc}
    \toprule
    & Cartésien & Cylindrique & Voronoï & Physique\\
    \midrule
    Écart moyen (pts) & 20{,}7 & 27{,}7 & 10{,}2 & \textbf{6{,}4}\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item Le choix de la discrétisation spatiale conditionne fortement la fidélité du modèle : les maillages statistiques (Voronoï, physique) surpassent nettement les maillages géométriques en termes d'erreur de prédiction (6,4 et 10,2 points d'écart de RSD contre 20,7 pour le cartésien et 27,7 pour le cylindrique) ; le cas cylindrique montre que le respect de la géométrie de l'enceinte ne garantit pas la pertinence du maillage vis-à-vis de la ségrégation.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccccc}
    ...
    \midrule
    Cartésien   & 91{,}1 & 92{,}4 & 104{,}3 & \textbf{76{,}7} & 34{,}1 & 38{,}4 & 75{,}5\\
    Cylindrique & 97{,}6 & 95{,}1 & 95{,}6  & \textbf{62{,}8} & 85{,}2 & 23{,}5 & 40{,}0\\
