...
La matrice de transition obtenue pour un découpage cylindrique à 24 cellules ($n_r = 3$, $n_\theta = 8$) est donnée à la figure~\ref{fig:matrice_cylindrique}. La structure en blocs, alignée sur la numérotation radiale $\to$ angulaire, traduit la physique de l'écoulement : les transitions dominantes relient des cellules angulairement voisines --- signature de l'entraînement en rotation solide dans la zone passive ---, tandis que les cellules radialement internes échangent avec les cellules externes via l'écoulement de surface. On note également des colonnes à forte probabilité concentrée : elles correspondent aux cellules du cœur du lit, dont les particules sont transportées en bloc vers une destination quasi déterministe en un tour de tambour. À l'inverse du découpage cartésien, aucune colonne vide n'apparaît : chaque cellule est visitée durant l'apprentissage, et la condition de stochasticité en colonne~\eqref{eq:stochastique} est vérifiée sur l'ensemble de la matrice.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/matrice_cylindrique.png}
  \caption{Matrice de transition obtenue pour le découpage cylindrique (24 cellules, $n_r = 3$, $n_\theta = 8$, mode à aire constante). La structure en blocs suit la numérotation radiale $\to$ angulaire ; la condition de stochasticité en colonne est vérifiée sur toutes les colonnes.}
...
Le tableau~\ref{tab:erreur_methodes} rassemble, pour les trois méthodes statistiquement les plus contrastées, l'écart temporel moyen $|\text{Markov}-\text{DEM}|$, l'écart absolu toutes cellules confondues et l'erreur RMS.
...
La comparaison des trois rangées révèle des différences substantielles tant en amplitude qu'en morphologie de l'erreur. Le modèle de Markov est donc fortement conditionné par la manière dont l'espace est partitionné : la robustesse de la prédiction ne dépend pas uniquement de la physique du mélange, mais aussi --- et peut-être surtout --- du choix de maillage. Les méthodes statistiques, qui adaptent les cellules à l'occupation réelle du mélangeur, réduisent l'erreur par rapport à la référence cartésienne ; le découpage cylindrique, sans atteindre la finesse des méthodes statistiques, corrige déjà l'essentiel des défauts du cartésien en respectant la géométrie de l'enceinte.
...
\paragraph{Rôle déterminant de la discrétisation.} Un maillage purement géométrique, bien que simple à mettre en œuvre, peut produire des cellules peu représentatives de la distribution effective des grains : le découpage cartésien crée des cellules de coin vides, quand le découpage cylindrique, en épousant l'enceinte, garantit déjà des cellules toutes physiquement pertinentes. Les approches statistiques --- Voronoï, physique --- vont plus loin en adaptant les partitions à la configuration réelle du lit et réduisent l'erreur de prédiction, comme en témoigne le tableau~\ref{tab:erreur_methodes}.
...
\begin{itemize}
  ...
  \item Le choix de la discrétisation spatiale conditionne fortement la fidélité du modèle : les maillages statistiques (Voronoï, physique) surpassent les maillages géométriques en termes d'erreur de prédiction, et le découpage cylindrique corrige les défauts géométriques du cartésien.
...
Ce choix répond à un double impératif. D'une part, $\tau$ doit être suffisamment grand pour que les particules aient le temps de changer de cellule entre deux observations : un $\tau$ trop court produit des matrices quasi identitaires (fortes diagonales artificielles) qui sous-estiment la cinétique du mélange. D'autre part, $\tau$ doit rester suffisamment court pour résoudre la dynamique du phénomène : un $\tau$ trop long saute des étapes de la cinétique et lisse artificiellement les courbes de RSD. L'étude paramétrique de la figure~\ref{fig:etude_tau}, menée pour des valeurs de $\tau$ allant de 10 à 1000 sur le découpage physique, illustre ce compromis : les petites valeurs de $\tau$ précipitent la convergence du RSD markovien vers zéro (sur-estimation de la vitesse de mélange), tandis que les grandes valeurs dégradent la résolution temporelle de la prédiction (courbes anguleuses, peu de points). Le choix $\tau = 157$, en plus de sa signification physique --- un tour de tambour ---, se situe dans la gamme intermédiaire qui préserve à la fois la cinétique et la résolution.
...
\begin{figure}[H]
  ...
  \begin{minipage}{\textwidth}
    ...
    \subcaption*{(b) Petites particules}
  \end{minipage}
  \caption{Influence du pas de temps de Markov $\tau$ sur la cinétique de mélange prédite (découpage physique, 30 cellules) : RSD de la DEM (noir) et prédictions markoviennes pour $\tau$ de 10 à 1000 centièmes de seconde.}
  \label{fig:etude_tau}
\end{figure}
...
Le paramètre $NLT$ contrôle le nombre de blocs d'apprentissage sur lesquels les probabilités de transition sont observées, puis moyennées (chaîne homogène) ou conservées séparément (chaîne inhomogène). L'étude de sensibilité compare des modèles construits avec $NLT = 1$ (un seul tour d'apprentissage) et $NLT = 50$, toutes choses égales par ailleurs (découpage physique, 10 cellules, $\tau = step = start = 157$).

La figure~\ref{fig:etude_nlt_ecart} montre l'écart $|\text{Markov} - \text{DEM}|$ dans les deux cas. Avec $NLT = 1$, l'écart RMS sur les petites particules plafonne autour de 5 à 10 particules avec des cellules durablement mal prédites (bandes rouges persistantes sur la carte des écarts) : la matrice apprise sur un unique tour reste tributaire de la configuration particulière de ce tour. Avec $NLT = 50$, l'écart RMS retombe dans la gamme 2 à 5, et les erreurs résiduelles se concentrent sur des épisodes courts plutôt que sur des cellules entières : la moyenne sur de nombreux blocs a gommé le bruit statistique de l'apprentissage. La figure~\ref{fig:etude_nlt_teneur} confirme ce constat sur l'évolution de la teneur en petites particules : la prédiction $NLT = 50$ (chaîne inhomogène) suit sensiblement mieux les trajectoires DEM, cellule par cellule, que la prédiction $NLT = 1$.
...
Le paramètre $start$ fixe le premier instant DEM utilisé pour la phase d'apprentissage --- et, par voie de conséquence, l'instant initial de la prédiction. Le début de simulation correspond au régime transitoire de mise en mouvement du lit : le RSD de la concentration en petites particules y chute brutalement depuis sa valeur initiale (configuration séparée) avant de fluctuer autour d'un palier, atteint après environ un tour de tambour. Apprendre les probabilités de transition sur cette phase transitoire biaiserait la matrice, qui encoderait une dynamique non représentative du mélange établi.

L'étude comparative porte sur trois valeurs de $start$ --- 157 (un tour), 314 (deux tours) et 1570 (dix tours) --- pour le découpage de Voronoï à 10 cellules, l'espèce des grandes particules et des paramètres par ailleurs identiques ($NLT = 1$, $\tau = step = 157$). La figure~\ref{fig:etude_start} montre l'écart $|\text{Markov} - \text{DEM}|$ pour les trois valeurs, et le tableau~\ref{tab:etude_start} en synthétise les niveaux d'écart RMS en régime établi.
