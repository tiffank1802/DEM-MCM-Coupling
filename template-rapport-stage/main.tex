...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/schema_forces_tambour.png}
  \caption{Bilan des actions mécaniques sur une particule du lit granulaire : force de contact normale $\mathbf{F}_{n,ij}$ et tangentielle $\mathbf{F}_{t,ij}$ au contact inter-grains, moment de rotation $\mathbf{M}_{r,ij}$, pesanteur $m_i\mathbf{g}$ et frottement tangentiel à la paroi entraînante.}
...
Plutôt que de suivre individuellement chaque grain, l'approche stochastique regroupe les particules dans des cellules issues d'une discrétisation spatiale de la zone utile du mélangeur comme le montre la figure~\ref{fig:maillage_melangeur},
...
\paragraph{Paramètres de construction et justification.} Quatre paramètres gouvernent la construction de la matrice ; leurs valeurs retenues sont rassemblées au tableau~\ref{tab:parametres} et la figure~\ref{fig:schema_parametres} en donne la chronologie sur l'axe des instants DEM :

\begin{itemize}[itemsep=3pt]
  \item $\boldsymbol{\tau} = 157$ centièmes de seconde : le pas de temps de Markov est calé sur la durée d'un tour complet du tambour tournant à $\omega = \SI{4}{rad/s}$, soit $2\pi/\omega \approx \SI{1.57}{s}$. Ce choix, physiquement interprétable, garantit que les particules ont le temps de se déplacer d'une cellule à l'autre entre deux observations (justification détaillée en annexe~\ref{annexe:tau}) ;
  \item $\boldsymbol{start} = 157$ : le premier instant utilisé pour l'apprentissage est choisi au début du régime permanent. Le début de simulation (régime transitoire) est volontairement écarté : les tout premiers instants correspondent à la mise en mouvement du lit, durant laquelle les probabilités de transition ne sont représentatives ni du mélange établi ni de la ségrégation en cours de formation ; apprendre sur cette phase biaiserait la matrice. L'étude comparative sur plusieurs valeurs de $start$ est donnée en annexe~\ref{annexe:start} ;
  \item $\boldsymbol{step} = 157$ : lorsque plusieurs blocs d'apprentissage sont utilisés ($NLT > 1$), $step$ désigne l'écart entre les débuts de deux blocs consécutifs. Prendre $step = \tau$ fait se succéder les blocs sans recouvrement ni trou, chaque bloc correspondant à un tour de tambour supplémentaire (justification en annexe~\ref{annexe:step}) ;
  \item $\boldsymbol{dt}$ : à l'intérieur d'un bloc, les paires d'instants $(t,\, t+\tau)$ sont échantillonnées tous les $dt$ pas de temps DEM. Cette stratégie de raffinage temporel entre les pas de temps démultiplie le nombre de paires observées : elle assure que chaque cellule source est visitée suffisamment souvent au cours de l'apprentissage, ce qui évite les colonnes vides --- donc les divisions par zéro et les \textit{NaN} --- dans la matrice de transition.
\end{itemize}

\begin{table}[H]
  \centering
  \caption{Paramètres de construction du modèle de Markov et valeurs retenues (en centièmes de seconde pour les paramètres temporels). Les justifications quantitatives sont données en annexe~\ref{annexe:parametres}.}
  \label{tab:parametres}
  \begin{tabular}{llll}
    \toprule
    Paramètre & Valeur & Signification & Justification\\
    \midrule
    $\tau$ & 157 & pas de temps de Markov & un tour de tambour (annexe~\ref{annexe:tau})\\
    $start$ & 157 & début de l'apprentissage & début du régime permanent (annexe~\ref{annexe:start})\\
    $step$ & 157 & écart entre blocs & blocs contigus, un tour chacun (annexe~\ref{annexe:step})\\
...
La figure~\ref{fig:comparaison_methodes_rsd} confronte, pour les quatre méthodes de découpage à \textbf{nombre de cellules identique} (10 cellules chacune : cartésien $5 \times 2 \times 1$, cylindrique $n_r = 2$, $n_\theta = 5$, Voronoï et physique $K = 10$) et à paramètres d'apprentissage identiques ($\tau = 157$, deux blocs), le RSD de la teneur en petites particules prédit par la chaîne homogène à la référence DEM. Cette égalité de résolution garantit que les écarts observés proviennent bien de la \textit{forme} des cellules --- c'est-à-dire de la méthode de découpage --- et non d'une différence de finesse de maillage, chaque cellule contenant en moyenne une centaine de particules dans les quatre cas. L'écart moyen $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ sur toute la fenêtre de prédiction est reporté dans chaque cadran et synthétisé au tableau~\ref{tab:ecart_rsd_methodes}. La hiérarchie est nette : le découpage physique (6,4 points d'écart) et le découpage de Voronoï (10,2 points) suivent correctement le niveau d'homogénéisation atteint par la DEM, tandis que les maillages géométriques --- cartésien (21,4 points) et cylindrique (26,7 points) --- s'en écartent durablement, leurs cellules moyennant des zones de dynamiques hétérogènes.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/comparaison_methodes_rsd.png}
  \caption{Influence de la méthode de découpage sur la prédiction (données DEM réelles) : RSD DEM vs prédiction markovienne homogène pour les quatre méthodes, à nombre de cellules identique (10 cellules) et à paramètres identiques ($\tau = 157$, deux blocs d'apprentissage). L'écart moyen est indiqué dans le titre de chaque cadran.}
  \label{fig:comparaison_methodes_rsd}
\end{figure}

\begin{table}[H]
  \centering
  \caption{Écart moyen $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ (en points de \%) sur la fenêtre de prédiction, par méthode de découpage à nombre de cellules identique (10 cellules, chaîne homogène, $\tau = 157$).}
  \label{tab:ecart_rsd_methodes}
  \begin{tabular}{lcccc}
    \toprule
    & Cartésien & Cylindrique & Voronoï & Physique\\
    \midrule
    Écart moyen (pts) & 21{,}4 & 26{,}7 & 10{,}2 & \textbf{6{,}4}\\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item Le choix de la discrétisation spatiale conditionne fortement la fidélité du modèle : à nombre de cellules identique, les maillages statistiques (Voronoï, physique) surpassent nettement les maillages géométriques en termes d'erreur de prédiction (6,4 et 10,2 points d'écart de RSD contre 21,4 pour le cartésien et 26,7 pour le cylindrique) ; le cas cylindrique montre que le respect de la géométrie de l'enceinte ne garantit pas la pertinence du maillage vis-à-vis de la ségrégation.
...
Cette annexe rassemble les études comparatives qui justifient les valeurs des paramètres de construction du modèle (tableau~\ref{tab:parametres} du corps du texte) : le pas de temps de Markov $\tau$ (\S\ref{annexe:tau}), le nombre de blocs d'apprentissage $NLT$ (\S\ref{annexe:nlt}), l'instant de départ $start$ (\S\ref{annexe:start}) et l'écart entre blocs $step$ (\S\ref{annexe:step}). Sauf mention contraire, tous les temps sont exprimés en centièmes de seconde (pas d'extraction DEM).

% ------------------------------------------------------------
\subsection{Choix du pas de temps de Markov \texorpdfstring{$\tau = 157$}{tau = 157}}
\label{annexe:tau}
...
Ce choix répond à un double impératif. D'une part, $\tau$ doit être suffisamment grand pour que les particules aient le temps de changer de cellule entre deux observations : un $\tau$ trop court produit des matrices quasi identitaires (fortes diagonales artificielles) qui figent la prédiction. D'autre part, $\tau$ doit rester suffisamment court pour résoudre la dynamique du phénomène : un $\tau$ trop long saute des étapes de la cinétique et lisse artificiellement les courbes de RSD. L'étude paramétrique de la figure~\ref{fig:etude_tau}, menée sur les données DEM pour des valeurs de $\tau$ allant de 10 à 1000 (découpage physique, 10 cellules, RSD de la teneur en petites particules), illustre ce compromis : les très petites valeurs ($\tau = 10$, $25$) figent le RSD prédit à un niveau élevé --- entre deux observations si rapprochées, les particules n'ont pas quitté leur cellule et la matrice, quasi identitaire, n'encode presque aucun échange ---, tandis que les grandes valeurs ($\tau = 500$, $1000$) dégradent la résolution temporelle de la prédiction (segments anguleux, peu de points). Le choix $\tau = 157$ (courbe épaisse), en plus de sa signification physique --- un tour de tambour ---, se situe dans la gamme intermédiaire qui suit correctement la descente initiale du RSD DEM et son niveau de palier.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/etude_tau_rsd.png}
  \caption{Influence du pas de temps de Markov $\tau$ sur la cinétique de mélange prédite, calculée sur les données DEM (découpage physique, 10 cellules) : RSD DEM de la teneur en petites particules (noir) et prédictions markoviennes homogènes pour $\tau$ de 10 à 1000 centièmes de seconde. La courbe épaisse correspond à la valeur retenue $\tau = 157$.}
  \label{fig:etude_tau}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{\textwidth}
    ...
    \subcaption*{(b) Petites particules}
  \end{minipage}
  \caption{Étude complémentaire de l'influence de $\tau$ (découpage physique, 30 cellules) : RSD de la DEM (noir) et prédictions markoviennes pour $\tau$ de 10 à 1000 centièmes de seconde.}
...
L'étude de sensibilité menée sur les données DEM (figure~\ref{fig:etude_nlt_courbe}) balaie $NLT$ de 1 à 18 (découpage physique, 10 cellules, $\tau = step = 157$) et mesure pour chaque valeur l'erreur moyenne $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ sur toute la fenêtre de prédiction. Deux régimes apparaissent : pour les petites valeurs ($NLT = 2$ à $5$), l'erreur fluctue entre 6 et 6,6 points --- chaque bloc supplémentaire modifie sensiblement la matrice moyenne, signe que la statistique n'est pas encore stabilisée --- puis, à partir de $NLT \gtrsim 8$, l'erreur décroît de façon monotone (5,6 points à $NLT = 8$, 4,7 points à $NLT = 18$) : la moyenne sur de nombreux blocs gomme le bruit statistique de l'apprentissage et la matrice converge vers la dynamique moyenne du régime permanent. On note que $NLT = 1$ peut ponctuellement donner une erreur faible : la matrice épouse alors la configuration particulière de son unique tour d'apprentissage, mais sans garantie de robustesse --- ce résultat dépend fortement du tour choisi, comme le confirme la sensibilité au paramètre $start$ (annexe~\ref{annexe:start}).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{figures/etude_nlt_erreur.png}
  \caption{Étude de sensibilité au nombre de blocs d'apprentissage $NLT$, calculée sur les données DEM (découpage physique, 10 cellules, $\tau = step = 157$) : erreur moyenne $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ sur la fenêtre de prédiction.}
...
\begin{figure}[H]
  ...
  \end{minipage}
  \caption{Influence du nombre de blocs d'apprentissage $NLT$ sur la qualité du modèle (découpage physique, 10 cellules, $\tau = 157$) : écart temporel RMS $|\text{Markov}-\text{DEM}|$ (gauche) et carte des écarts absolus par cellule (droite).}
...
% ------------------------------------------------------------
\subsection{Choix de l'instant de départ \texorpdfstring{$start = 157$}{start = 157}}
\label{annexe:start}

Le paramètre $start$ fixe le premier instant DEM utilisé pour la phase d'apprentissage --- et, par voie de conséquence, l'instant initial de la prédiction. Le début de simulation correspond au régime transitoire de mise en mouvement du lit. La figure~\ref{fig:etude_start_rsd}, calculée sur les données DEM, montre le RSD de la teneur en petites particules durant les 12 premières secondes pour les quatre méthodes de découpage : quelle que soit la méthode, le RSD part de valeurs très élevées (90 à 110 \%, configuration initiale séparée), culmine durant la mise en mouvement du lit (jusqu'à 120 \% pour les maillages géométriques), puis amorce sa décroissance vers le palier du régime établi. La ligne verticale $start = 157$ --- un tour de tambour --- marque la fin de cette phase transitoire : au-delà, la décroissance du RSD est engagée pour les quatre découpages et les probabilités de transition observées deviennent représentatives du mélange établi. Apprendre les probabilités sur la zone grisée (régime transitoire) biaiserait la matrice, qui encoderait une dynamique de démarrage non représentative. Le tableau~\ref{tab:etude_start_rsd} donne les valeurs du RSD à quelques instants clefs.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/etude_start_rsd.png}
  \caption{Justification du choix $start = 157$ : RSD DEM de la teneur en petites particules durant les 12 premières secondes, pour les quatre méthodes de découpage. La zone grisée matérialise le régime transitoire de mise en mouvement du lit ; la ligne pointillée marque $start = 157$ (un tour de tambour).}
  \label{fig:etude_start_rsd}
\end{figure}

\begin{table}[H]
  \centering
  \caption{RSD DEM de la teneur en petites particules (\%) à quelques instants (centièmes de seconde), par méthode de découpage : le régime transitoire ($t < 157$) présente des valeurs hautes et erratiques ; la décroissance s'installe au-delà.}
  \label{tab:etude_start_rsd}
  \begin{tabular}{lccccccc}
    \toprule
    $t$ (cs) & 0 & 50 & 100 & \textbf{157} & 300 & 600 & 1000\\
    \midrule
    Cartésien   & 87{,}2 & 101{,}0 & 87{,}7 & \textbf{65{,}7} & 110{,}8 & 36{,}0 & 20{,}8\\
    Cylindrique & 107{,}3 & 96{,}3 & 119{,}8 & \textbf{66{,}0} & 65{,}7 & 88{,}9 & 76{,}2\\
    Voronoï     & 62{,}1 & 89{,}5 & 29{,}7  & \textbf{61{,}7} & 23{,}9 &  9{,}1 & 19{,}6\\
    Physique    & 107{,}1 & 90{,}4 & 88{,}6 & \textbf{68{,}3} & 31{,}6 & 15{,}1 & 12{,}3\\
    \bottomrule
  \end{tabular}
\end{table}

L'étude comparative est complétée par trois valeurs de $start$ --- 157 (un tour), 314 (deux tours) et 1570 (dix tours) --- pour le découpage de Voronoï à 10 cellules, l'espèce des grandes particules et des paramètres par ailleurs identiques ($NLT = 1$, $\tau = step = 157$). La figure~\ref{fig:etude_start} montre l'écart $|\text{Markov} - \text{DEM}|$ pour les trois valeurs, et le tableau~\ref{tab:etude_start} en synthétise les niveaux d'écart RMS en régime établi.
...
\begin{figure}[H]
  ...
  \begin{minipage}{\textwidth}
    ...
    \subcaption*{(c) $start = 1570$}
  \end{minipage}
  \caption{Étude d'influence du paramètre $start$ (découpage de Voronoï, 10 cellules, grandes particules, $NLT = 1$, $\tau = step = 157$) : écart temporel RMS et carte des écarts absolus par cellule pour $start = 157$, $314$ et $1570$.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    \toprule
    & $start = 157$ & $start = 314$ & $start = 1570$\\
    \midrule
    ...
    Fenêtre de prédiction restante & maximale & $-157$ pas & $-1413$ pas\\
    \bottomrule
  \end{tabular}
\end{table}

Deux constats se dégagent. D'une part, les trois valeurs de $start$ conduisent à des niveaux d'écart quasi identiques en régime établi : dès $start = 157$, le système a atteint son régime permanent et les probabilités apprises sont stables --- retarder davantage le départ n'apporte aucun gain de précision. D'autre part, retarder le $start$ ampute d'autant la fenêtre de prédiction et rapproche l'apprentissage de la fin de la simulation. La valeur $start = 157$ --- un tour de tambour, cohérente avec le pas de temps $\tau$ --- est donc retenue : elle marque le début du régime permanent tout en préservant la plus longue fenêtre de validation possible.

% ------------------------------------------------------------
\subsection{Choix de l'écart entre blocs \texorpdfstring{$step = 157$}{step = 157}}
\label{annexe:step}
...
\begin{itemize}[itemsep=2pt]
  ...
  \item $step = \tau = 157$ fait se succéder les blocs de manière contiguë et sans recouvrement : chaque bloc correspond exactement à un tour de tambour supplémentaire, chaque transition observée est comptée une seule fois, et la fenêtre d'apprentissage reste compacte.
\end{itemize}

L'étude de sensibilité menée sur les données DEM (figure~\ref{fig:etude_step_courbe}) balaie $step$ de 40 à 471 (découpage physique, 10 cellules, $NLT = 3$, $\tau = 157$) et mesure l'erreur moyenne $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$. On observe que les valeurs intermédiaires non commensurables avec la période du tambour ($step = 80$) dégradent l'erreur (8,6 points) : les blocs échantillonnent alors des phases de rotation différentes, et leur moyenne mélange des configurations angulaires hétérogènes. Les valeurs commensurables avec la période ($step = 157$, $314$) ramènent l'erreur autour de 6 points, chaque bloc couvrant un nombre entier de tours ; au-delà ($step = 471$), l'erreur remonte, la fenêtre totale d'apprentissage s'étirant sans gain statistique. La petite valeur $step = 40$ donne l'erreur la plus basse sur ce cas particulier, mais au prix d'un recouvrement partiel des fenêtres d'observation des blocs, qui corrèle les blocs entre eux et fragilise l'interprétation de la moyenne --- et, pour les chaînes inhomogènes, désynchronise les matrices de la période physique.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{figures/etude_step_erreur.png}
  \caption{Étude de sensibilité à l'écart entre blocs $step$, calculée sur les données DEM (découpage physique, 10 cellules, $NLT = 3$, $\tau = 157$) : erreur moyenne $|\mathrm{RSD}_{\text{Markov}} - \mathrm{RSD}_{\text{DEM}}|$ sur la fenêtre de prédiction. La ligne pointillée marque la valeur retenue $step = \tau = 157$.}
  \label{fig:etude_step_courbe}
\end{figure}

Pour les chaînes inhomogènes, le choix $step = \tau$ prend une signification supplémentaire : la matrice $\mathbf{P}^{(k)}$ du bloc $k$ est exactement la matrice du $k$-ième tour de tambour suivant le départ, ce qui rend la séquence de matrices directement interprétable --- et synchronisée avec la prédiction de l'équation~\eqref{eq:prediction_inhomogene}, chaque pas de prédiction consommant la matrice du bloc correspondant. La valeur $step = 157$ est donc adoptée pour tous les modèles multi-blocs du mémoire, la cohérence $\tau = step = start = 157$ alignant l'ensemble de la chronologie d'apprentissage sur la période physique du tambour.
