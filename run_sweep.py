...
def run_experiment(config, partitioner, files, fs, device):
    ...
    # Sélection des timesteps
    all_indices = list(
        range(config.start_index, len(files) - 1, config.step_size)
    )
    selected = all_indices[: config.nlt]

    if len(selected) < config.nlt:
        print(
            f"   ⚠️  {len(selected)} timesteps disponibles "
            f"(demandé: {config.nlt})"
        )

    if len(selected) == 0:
        raise ValueError("Aucun timestep sélectionné")

    # Accumulateur GPU
    P_acc = torch.zeros((n_states, n_states), dtype=torch.float64, device=device)

    for idx in tqdm(selected, desc="   Timesteps", leave=False):
        # Lecture
        with fs.open(files[idx], "rb") as f:
            df_prev = pl.read_csv(f)
        with fs.open(files[idx + 1], "rb") as f:
            df_curr = pl.read_csv(f)

        # États via le partitionneur
