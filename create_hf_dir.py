...
for dossier in dossiers:
    if all(j in dossier.get('name', '') for j in ["voronoi_", "NLT10", "40cells"]):
        path=dossier['name']
P=np.array([])
config={}
...
fig.tight_layout()
plt.savefig(f"/teamspace/studios/this_studio/MyStudio/outputs/images/transition_{path.replace("buckets/ktongue/DEM_MCM/Experiments/","")}.png")
print(S)
states_history=[S]
...
fig.tight_layout()
plt.legend()
plt.savefig(f'/teamspace/studios/this_studio/MyStudio/outputs/images/etats_{path.replace("buckets/ktongue/DEM_MCM/Experiments/","")}.png')

np.savetxt(f'/teamspace/studios/this_studio/MyStudio/outputs/fichiers/transition_{path.replace("buckets/ktongue/DEM_MCM/Experiments/","")}.txt',states_history)
