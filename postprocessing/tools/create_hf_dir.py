...
for dossier in dossiers:
    if all(
        ...
        for j in ["voronoi_30cells_NLT200_step50_dt2_tau100_start250_d0008"]
    ):
        path = dossier["name"]
...
partitioner = create_partitioner(
    config.get("method", ""), **config.get("method_kwargs", "")
)
...
def screenshot(coords: np.ndarray, states: np.ndarray) -> None:
    mesh = pv.PolyData(coords)
    mesh.point_data["partitions"] = states
    mesh.point_data.set_array(
        data=timestep_dict[start][["Diameter"]].to_numpy(), name="Diameter"
    )
...
initial_coords = timestep_dict[start][
    ["coordinates:0", "coordinates:1", "coordinates:2"]
].to_numpy()
partitioner.fit(initial_coords)
states = partitioner.compute_states(
    initial_coords[:, 0], initial_coords[:, 1], initial_coords[:, 2]
)
