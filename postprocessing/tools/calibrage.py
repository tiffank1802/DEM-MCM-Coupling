...
pv.start_xvfb()
pv.OFF_SCREEN = True
LOCAL_OUTPUT = "/teamspace/studios/this_studio/DEM-MCM-Coupling/outputs"
...
def load_npy(fs: HfFileSystem, path: str, filename: str) -> np.ndarray:
    with fs.open(f"{path}/{filename}", "rb") as f:
        return np.load(io.BytesIO(f.read()))


def load_json(fs: HfFileSystem, path: str, filename: str) -> dict:
    with fs.open(f"{path}/{filename}", "r") as f:
        return json.load(f)


config = load_json(fs, path, "stats.json")  # stats contient species_list
species_list = config.get(
    "species_list", ["small", "large"]
)  # tailles des particules(par défaut deux tailles)
exp_config = load_json(
    fs, path, "config.json"
)  # configurations du modèle (spatial et temporel)

start = exp_config.get(
    "start_index", 250
)  # start (debut de l'apprentissage) et instant du partitionnement
tau = exp_config.get("tau", 50)  # pas de temps de markov
tau_dem = (
    tau  # S_matrix est déjà échantillonnée à chaque timestep DEM # pas de temps dem
)
...
for species in species_list:
    ...
    times = load_npy(
        fs, path, f"times_{species}.npy"
    )  # indices DEM triés tous les pas de temps de la DEM au cas où on travaillerait avec des données differentes
    species_data[species] = {"P": P, "S_matrix": S_matrix, "times": times}
    print(
        f"   ✅ '{species}' — P:{P.shape}, S_matrix:{S_matrix.shape}, "
...
def propagate_markov(
    ...
    row_start = np.searchsorted(
        times, start_idx
    )  # indice de markov de début dans les indices de la dem
    times_full = times[row_start:]  # tous les timesteps DEM depuis start

    # On propage à chaque pas tau (pas tous les timesteps DEM)
    markov_indices = np.arange(
        0, len(times_full), tau
    )  # indices pour la prédiction de markov ie séparé de tau
    times_markov = times_full[markov_indices]  # les temps correspondant à ces indices

    S = S0.copy().astype(float)
    trajectory = [
        S.copy()
    ]  # est une matrice de taille (times_markov,n_states) ou de taille (n_steps,n_states)
...
for species, data in species_data.items():
    ...
    print(
        f"   🔁 Markov '{species}' : {traj.shape[0]} pas | "
...
# ── RSD par espèce ───────────────────────────────────────────────────────────
def compute_rsd_from_S(S_traj: np.ndarray) -> np.ndarray:
    """
...
def compute_rsd_concentration(S_small: np.ndarray, S_large: np.ndarray) -> np.ndarray:
    """
...
fig, axes = plt.subplots(
    1, len(species_list) + 1, figsize=(5 * (len(species_list) + 1), 4)
)
...
if len(species_list) == 2:
    ...
    n_min = min(
        len(markov_results[sp_a]["times_markov"]),
    ...
    ax_conc.plot(
        times_markov_common, rsd_conc_markov, "o-", markersize=3, label="Markov"
    )
...
fig.tight_layout()
plt.savefig(
    f"{LOCAL_OUTPUT}/images/rsd/rsd_{short_path}.png", dpi=150, bbox_inches="tight"
)
plt.close(fig)
print("   💾 Figure RSD sauvegardée")
...
axes[0].axvline(
    eigvals[1].real, color="orange", lw=1, label=f"λ1={eigvals[1].real:.4f}"
)
...
# Matrice P
im = axes[2].imshow(P, aspect="auto", cmap="viridis", vmin=0, vmax=P.max())
...
def rsd_concentration(S_small: np.ndarray, S_large: np.ndarray) -> float:
    total = S_small + S_large
...
ax.plot(times_markov, rsd_dem, "o-", markersize=3, lw=2, label="DEM", alpha=0.7)
ax.set_xlabel("Temps (centièmes de secondes)")
...
plt.tight_layout()
plt.savefig(
    f"{LOCAL_OUTPUT}/images/rsd/rsd_concentration_{short_path}.png",
    dpi=150,
    bbox_inches="tight",
)
# #plt.show()
...
def rsd_model(t: np.ndarray, RSD_inf: float, RSD_0: float, tau_c: float) -> np.ndarray:
...
try:
    popt_mc, _ = curve_fit(
        rsd_model, t_markov_fit, rsd_markov_full, p0=p0_mc, maxfev=10000
    )
...
print(
    f"🔄 Recalibrage : tau original (t_l) = {tau}, tau calibré (t_m) = {tau_calibrated:.2f}"
)
...
ax.plot(
    t_fit_dem_plot,
    rsd_model(t_fit_dem_plot - start, *popt_dem),
    "k--",
    lw=1.5,
...
try:
    popt_mc_cal, _ = curve_fit(
        rsd_model, t_markov_calibrated_fit, rsd_markov_full, p0=p0_mc, maxfev=10000
    )
    t_fit_mc_plot = (
        np.linspace(t_markov_calibrated_fit[0], t_markov_calibrated_fit[-1], 200)
        + start
    )
    ax.plot(
        t_fit_mc_plot,
        rsd_model(t_fit_mc_plot - start, *popt_mc_cal),
        "r--",
        lw=1.5,
...
ax.set_title(
    f"Recalibrage du pas de temps Markov (Méthode de Zhou)\n{short_path}", fontsize=14
)
